...
function FaqItem({ q, a, index }: FaqItemType & { index: number }) {
  ...
  return (
    <motion.div
      ...
      className="bg-white border border-gray-200 rounded-xl shadow-sm dark:bg-stone-600/50 dark:!border-stone-800">
      <button
        onClick={() => setOpen((v) => !v)}
        className="w-full flex items-center justify-between px-6 py-5 text-left">
        <span className="text-sm font-semibold text-gray-900 dark:text-slate-200">{q}</span>
      ...
      <AnimatePresence initial={false}>
        {open && (
          <motion.div
            ...
            <p className="px-6 pb-5 text-sm text-gray-600 dark:text-slate-300 leading-relaxed">
              {a}
            </p>
          </motion.div>
        )}
...
export default function FaqSection() {
  ...
  return (
    <section ref={ref} id="faq" className="py-24 bg-[#f5f6f8] dark:bg-stone-900">
